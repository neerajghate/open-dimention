...
async function fixture(t, { seed = false } = {}) {
  ...
  async function start() {
    app = await createAppServer({ dataPath, seed });
  ...
  return {
    ...
    async request(path, method = "GET", input) {
...
const node = (type = "note") => ({
  type,
  title: `A ${type}`,
  content: "A long thought\nwith new lines & <literal> markup.",
  x: -18.5,
  y: 80,
  z: 340,
  payload:
...
test("all four types create, update and survive a server restart; export equals stored graph", async (t) => {
  ...
  for (const type of ["note", "idea", "workflow", "table"]) {
    const created = await f.request("/nodes", "POST", node(type));
    assert.equal(created.status, 201);
    assert.ok(created.data.createdAt);
    const changed = { ...node(type), title: `Edited ${type}`, z: -450.75 };
    if (type === "workflow")
      changed.payload.steps = [
        { text: "Edited first", done: true },
        { text: "Added step", done: false },
      ];
    if (type === "table")
      changed.payload = {
        columns: ["Renamed", "Status", "Extra"],
        rows: [["changed", "ready", "persistent"]],
      };
    const updated = await f.request(
      "/nodes/" + created.data.id,
      "PATCH",
      changed,
    );
    assert.equal(updated.status, 200);
    expected.push(updated.data);
  }
  const edge = await f.request("/connections", "POST", {
    source: expected[0].id,
...
test("directed edges reject duplicates, self-links and missing nodes; Trash hides affected edges", async (t) => {
  const f = await fixture(t);
  const a = (await f.request("/nodes", "POST", node())).data;
  const b = (await f.request("/nodes", "POST", node("idea"))).data;
  ...
  const reverse = await f.request("/connections", "POST", {
    source: b.id,
    target: a.id,
  });
  assert.equal(reverse.status, 201);
  assert.equal(
    (await f.request("/connections", "POST", { source: a.id, target: a.id }))
      .status,
    400,
  );
  assert.equal(
    (
      await f.request("/connections", "POST", {
        source: a.id,
        target: "missing",
      })
    ).status,
  ...
  assert.equal(
    (await f.request("/connections/" + reverse.data.id, "DELETE")).status,
    200,
  );
  assert.equal(
    (await f.request("/connections/" + reverse.data.id, "DELETE")).status,
  ...
  assert.equal(
    (await f.request("/nodes/" + a.id, "PATCH", { title: "ghost" })).status,
    404,
  );
...
test("first initialization seeds once; deleting every type never recreates samples", async (t) => {
  ...
  const initial = (await f.request("/workspace")).data;
  assert.equal(initial.nodes.length, 6);
  assert.equal(new Set(initial.nodes.map((n) => n.type)).size, 4);
  assert.ok(new Set(initial.nodes.map((n) => n.z)).size > 3);
  assert.equal(
    initial.nodes.find((n) => n.type === "workflow").payload.steps.length,
    5,
  );
  await f.restart();
  assert.equal((await f.request("/workspace")).data.nodes.length, 6);
  for (const n of initial.nodes)
    assert.equal((await f.request("/nodes/" + n.id, "DELETE")).status, 200);
  await f.restart();
...
test("invalid node, workflow and table inputs produce controlled errors without writes", async (t) => {
  ...
  const inputs = [
    ...
    { ...node(), type: "other" },
    { ...node(), title: "  " },
    { ...node(), title: "a".repeat(161) },
    { ...node(), z: 5001 },
    { ...node(), y: "5" },
    { ...node(), content: "x".repeat(100001) },
    { ...node(), payload: [] },
    { ...node(), starred: "yes" },
    { ...node("workflow"), payload: { steps: [{ text: "test", done: 1 }] } },
    {
      ...node("workflow"),
      payload: {
        steps: Array.from({ length: 201 }, () => ({ text: "x", done: false })),
      },
    },
    { ...node("table"), payload: { columns: [], rows: [] } },
    { ...node("table"), payload: { columns: ["a"], rows: [["x", "y"]] } },
    { ...node("table"), payload: { columns: ["a"], rows: [[true]] } },
  ];
  for (const input of inputs) {
    const response = await f.request("/nodes", "POST", input);
    assert.equal(response.status, 400, JSON.stringify(input).slice(0, 150));
    assert.equal(typeof response.data.error, "string");
    assert.equal(response.data.stack, undefined);
  }
...
test("parameterized SQL safely stores SQL-like text and partial position updates preserve data", async (t) => {
  const f = await fixture(t);
  const title = "'; DROP TABLE nodes; --";
  const created = (
    await f.request("/nodes", "POST", { ...node("table"), title })
  ).data;
  const updated = (await f.request("/nodes/" + created.id, "PATCH", { x: 123 }))
    .data;
  assert.equal(updated.title, title);
  assert.deepEqual(updated.payload, created.payload);
  assert.equal(updated.x, 123);
  assert.equal(updated.z, created.z);
  assert.equal(
    (await f.request("/nodes/" + created.id, "PATCH", null)).status,
    400,
  );
...
test("bounded requests, content types, malformed JSON, origins and paths are checked", async (t) => {
  const f = await fixture(t);
  let response = await fetch(f.base + "/api/nodes", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: "{wrong",
  });
  assert.equal(response.status, 400);
  response = await fetch(f.base + "/api/nodes", {
    method: "POST",
  ...
  response = await fetch(f.base + "/api/nodes", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ title: "x".repeat(6 * 1024 * 1024) }),
  });
  assert.equal(response.status, 413);
  response = await fetch(f.base + "/api/workspace", {
  ...
  response = await fetch(f.base + "/server/store.mjs");
  assert.equal(response.status, 404);
  response = await fetch(f.base + "/");
  assert.equal(response.status, 200);
  assert.match(await response.text(), /Dimention/);
});
...
test("preliminary workflow text migrates to structured steps without losing other records", async (t) => {
  ...
  const db = new DatabaseSync(f.dataPath);
  db.prepare("UPDATE nodes SET payload=? WHERE id=?").run("{}", created.id);
  db.close();
  await f.restart();
  const migrated = (await f.request("/workspace")).data.nodes[0];
  assert.deepEqual(migrated.payload.steps, [
...
test("100-node graph persists without missing nodes or orphaned edges", async (t) => {
  ...
  const ids = [];
  for (let i = 0; i < 100; i++) {
    const created = await f.request("/nodes", "POST", {
      ...node(),
      title: `Thought ${i}`,
      x: i * 10,
      y: (i % 7) * 60,
  ...
  await f.request("/nodes/" + ids[50], "DELETE");
  const after = (await f.request("/workspace")).data;
  assert.equal(after.nodes.length, 99);
  assert.equal(after.edges.length, 97);
  assert.ok(
    after.edges.every(
      (e) =>
        after.nodes.some((n) => n.id === e.source) &&
        after.nodes.some((n) => n.id === e.target),
    ),
  );
});

test("Dim organization, Trash, restore and import HTTP routes persist as one world", async (t) => {
  const f = await fixture(t);
  const dim = (
    await f.request("/nodes", "POST", {
      type: "dim",
      title: "Room",
      payload: { color: "#8580ff" },
      x: 1000,
      y: 0,
      z: 0,
    })
  ).data;
  const doc = (await f.request("/nodes", "POST", node())).data;
  let organized = await f.request("/organize", "PATCH", {
    ids: [doc.id],
    dimId: dim.id,
    area: "desk",
  });
  ...
  assert.equal(
    (
      await f.request("/positions", "PATCH", {
        positions: [{ id: dim.id, x: 1100, y: 0, z: 0 }],
      })
    ).status,
    200,
  );
  const exported = (await f.request("/export")).data;
  const preview = await f.request("/import", "POST", {
    workspace: exported,
    preview: true,
  });
  assert.equal(preview.data.items, 2);
  assert.equal((await f.request("/workspace")).data.nodes.length, 2);
  assert.equal(
    (
  ...
  await f.request("/nodes/" + dim.id, "DELETE");
  assert.equal((await f.request("/trash")).data.nodes.length, 2);
  await f.restart();
  assert.equal((await f.request("/workspace")).data.nodes.length, 0);
  assert.equal(
    (await f.request("/trash/" + dim.id + "/restore", "POST")).status,
    200,
  );
  assert.equal(
